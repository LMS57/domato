...
def generate_new_sample(template, cgrammar):
    ...
    result = template
    handlers = False
    while '<C_MAIN_FUZZING>' in result:#where the code is placed in the template
        ...
        result = result.replace(
            '<C_MAIN_FUZZING>',
            cgrammar._generate_code(numlines),
            1
        )
...
def generate_samples(grammar_dir, outfiles):
    ...
    f = open(os.path.join(grammar_dir, 'template.txt'))
    template = f.read()
    f.close()
